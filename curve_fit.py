...
class pos_curve():
	def __init__(self):
		...
		self.data = pd.read_csv('posgen_data.csv')
	...
	def collect_curves(self):#fit self.curves
	...
	def vis_curve(self):
		...
		ax2 = sns.lineplot(x='face',y='scr_est',hue='phase',data=self.curves,ci=None,
							palette=pos_pal)
		for i, phase in enumerate(self.phases):
		...
		ax2.set_xlim(0,1);ax2.set_ylim(0,.8);sns.despine(ax=ax2,right=False)
		ax2.legend_.remove()
		...
		sns.barplot(x='coef',y='est',hue='phase',data=coefs_,dodge=True,palette=pos_pal,ci=None,ax=cplot)
		...
		sns.barplot(x='coef',y='est',hue='phase',data=Peak,dodge=True,palette=pos_pal,ci=None,ax=pplot)
